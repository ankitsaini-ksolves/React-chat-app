...
const PostPage = () => {
  ...
  const { isModalOpen, modalType, currentPost } = useSelector((state) => state.modal);
  ...
  return (
    <div className="container mt-4">
      <div className="flex input-group mb-3" style={{ position: 'relative' }}>
        ...
        />
        {searchTerm && (
          <span onClick={handleClearSearch} className="clear-icon" style={{ cursor: 'pointer' }}>
            <i className="mdi mdi-close-circle-outline mdi-24px mdi-dark mdi-inactive"></i>
          </span>
        )}
        <button className="btn btn-primary" onClick={() => handleShowModal('add')}>Add</button>
      </div>
            ...
            <div className="col-md-4" key={post.id}>
              <div className="card mb-4">
                ...
                <div className="card-body">
                  ...
                  <button onClick={() => dispatch(toggleModal({ type: 'edit', post }))} className=" m-1 btn btn-warning">Edit</button>
      ...
      {isModalOpen && <Modal handleClose={() => dispatch(toggleModal())} userId={user.id} modalType={modalType} currentPost={currentPost} />}
    </div>
  );
};
